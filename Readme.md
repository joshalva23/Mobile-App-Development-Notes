# Mobile App Development Course Notes

This repository contains structured notes on Android development, covering topics such as data storage, advanced programming concepts, hardware support, and security.

## 📚 Contents

### Unit 1

#### Files:

- [`Smart phone operating systems and smart phones applications`](Unit-1/SmartphoneOperatingSystem.md)
- [`Introduction to Android`](Unit-1/IntroToAndroid.md)
- [`Creating Android App Project`](Unit-1/CreateAndroidProject.md)
- [`Deploying the App`](Unit-1/DeployApp.md)
- [`Building a Layout`](Unit-1/BuildingALayout.md)
- [`Layouts, Views and Resources`](Unit-1/LayoutViewsResources.md)
- [`Text and Scrolling Views`](Unit-1/TextAndScrollingViews.md)
- [`Activities and Intents`](Unit-1/ActivitiesAndIntents.md)
- [`Activity Lifecycle`](Unit-1/ActivityLifecycle.md)
- [`Managing State`](Unit-1/ManagingState.md)
- [`Activties and Implicit Intents`](Unit-1/ActivitiesAndImplicitIntents.md)
- [`Android Studio Debugger`](Unit-1/AndroidStudioDebugger.md)
- [`Testing Android App`](Unit-1/TestingAndroidApp.md)
- [`Android Support Library`](Unit-1/AndroidSupportLibrary.md)

---

### Unit 2: User experience

#### Files:

- [`User Interaction`](Unit-2/UserInteraction.md)
- [`User Input Controls`](Unit-2/UserInputControls.md)
- [`Menus`](Unit-2/Menus.md)
- [`Screen Navigation`](Unit-2/ScreenNavigation.md)
- [`Recycler View`](Unit-2/RecyclerView.md)
- [`Delightful User Experience`](Unit-2/DelightfulUserExperience.md)
- [`Drawables`](Unit-2/Drawables.md)
- [`Styles and Themes`](Unit-2/StylesAndThemes.md)
- [`Material Design`](Unit-2/MaterialDesign.md)
- [`Testing App UI`](Unit-2/TestingAppUI.md)
- [`Testing the User Interface`](Unit-2/TestingUserInterface.md)

---

### Unit 3: Working in the Background

#### Files:
- [`AsyncTask and AsyncTaskLoader`](Unit-3/AsyncTaskandLoader.md)
- [`Connect To the Internet`](Unit-3/ConnectToInternet.md)
- [`Broadcast Receiver And Services`](Unit-3/BroadCastReceiversandServices.md)
- [`Scheduling and Managing Background Tasks`](Unit-3/SchedulingAndManagingBGTasks.md)

---

### Unit 4: All About Data 

#### Files:
- [`Preferences and Settings`](Unit-4/PreferencesAndSettings.md)
- [`Storing Data`](Unit-4/StoreData.md)
- [`Shared Preferences`](Unit-4/SharedPreferences.md)
- [`Storing Data SQLite`](Unit-4/StoringDataSQLite.md)
- [`SQLite`](Unit-4/SQLite.md)
- [`Sharing Data Content Providers`](Unit-4/SharingDataContentProviders.md)
- [`Advanced Android Programming`](Unit-4/AdvancedAndroidProgramming.md)
- [`Display WebPage and Maps`](Unit-4/DisplayWebPageandMaps.md)
- [`Communication with Email and SMS`](Unit-4/CommEmailInSMS.md)
- [`Sensors`](Unit-4/Sensors.md)


---

### Unit 5: Hardware Support & Devices

#### Files:
- [`Permissions and Libraries`](Unit-5/PermissionsandLibraries.md)
- [`Performance Security`](Unit-5/PerformanceSecurity.md)
- [`Firebase and AdMob`](Unit-5/FirebaseandAdMob.md)
- [`Publish Polish`](Unit-5/PublishPolish.md)
- [`Multiple Form Factors`](Unit-5/MultipleFormFactors.md)
- [`Google Services`](Unit-5/GoogleServices.md)

---

## 📌 How to Use
- Each `.md` file contains detailed explanations of the respective topics.
- Follow along with practical examples and code snippets to enhance your learning experience.


